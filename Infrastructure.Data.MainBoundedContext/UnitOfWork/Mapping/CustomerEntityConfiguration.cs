...
namespace Microsoft.Samples.NLayerApp.Infrastructure.Data.MainBoundedContext.UnitOfWork.Mapping
{
   ...
   internal class CustomerEntityConfiguration : EntityTypeConfiguration<Customer>
   {
      ...
      public CustomerEntityConfiguration()
      {
         ...
         this.Property(c => c.Company).HasMaxLength(200);

         this.Property(c => c.Telephone).HasMaxLength(25);

         this.Property(c => c.IsEnabled).IsRequired();

         this.Property(c => c.CreditLimit).IsRequired();
         ...
         this.HasRequired(c => c.Country) // 1..*
            .WithMany().HasForeignKey(c => c.CountryId).WillCascadeOnDelete(false);
