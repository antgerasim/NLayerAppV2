...
namespace Microsoft.Samples.NLayerApp.Infrastructure.Data.MainBoundedContext.ERPModule.Repositories
{
      ...
      ICustomerRepository
   {
      ...
      /// <summary>
      ///    <see
      ///       cref="Microsoft.Samples.NLayerApp.Domain.MainBoundedContext.ERPModule.Aggregates.CustomerAgg.ICustomerRepository" />
      /// </summary>
      /// <param name="id"></param>
      /// <returns></returns>
      public override Customer Get(Guid id)
      {
         if (id != Guid.Empty)
         {
            ...
            return set.Include(c => c.Picture).Where(c => c.Id == id).SingleOrDefault();
         }
         else
         {
            return null;
      ...
      public IEnumerable<Customer> GetEnabled(int pageIndex, int pageCount)
      {
         ...
         return
            currentUnitOfWork.Customers.Where(c => c.IsEnabled == true)
